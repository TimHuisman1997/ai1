...
Fringe insertValidSucc(Fringe fringe, Fringe prSt, int value, int action, State state) {
  State s;
  //printf("%d\n", state.value);
...
void search(int mode, int start, int goal) {
  ...
  while (!isEmptyFringe(fringe)) {
    ...
    fringe = insertValidSucc(fringe, prSt, value+1, 1, state); /* rule n->n + 1      */
    fringe = insertValidSucc(fringe, prSt, 2*value, 2, state); /* rule n->2*n        */
    fringe = insertValidSucc(fringe, prSt, 3*value, 3, state); /* rule n->3*n        */
    fringe = insertValidSucc(fringe, prSt, value-1, 4, state); /* rule n->n - 1      */
    fringe = insertValidSucc(fringe, prSt, value/2, 5, state); /* rule n->floor(n/2) */
    fringe = insertValidSucc(fringe, prSt, value/3, 6, state); /* rule n->floor(n/3) */
  }
  if (goalReached == 0) {
